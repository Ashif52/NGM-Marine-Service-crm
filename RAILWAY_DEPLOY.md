...
3.  **Configuration**:
    ...
    - **Start Command**: `npm run preview -- --host --port $PORT` (or use Railway's Static Site deployment if preferred).
...
4.  **Environment Variables**:
    ...
    - Add all `VITE_FIREBASE_*` variables from your local setup.
...
-   **Backend 404/500**: Check Railway logs. Ensure `FIREBASE_PRIVATE_KEY` is correct.

...
import { Search, Eye, CheckCircle, Plus, Loader2 } from 'lucide-react';
...
export function DGInCommunication() {
  ...
  const loadData = async () => {
    try {
      ...
      const [commsResponse, shipsResponse] = await Promise.all([
        dgCommunicationApi.getAll({ comm_type: 'incoming' }),
        shipsApi.getAllShips()
      ]);
      
      if (commsResponse.data) {
        // Filter explicitly to ensure we only get incoming communications
        const incomingCommunications = commsResponse.data.filter(comm => comm.comm_type === 'incoming');
        setCommunications(incomingCommunications);
        console.log('Loaded incoming communications:', incomingCommunications.length);
      }
  ...
  const handleAddResponse = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await dgCommunicationApi.addResponse(selectedComm.id, responseText, false);
      if (response.error) {
        throw new Error(response.error);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  <DialogTitle>Record Incoming Communication</DialogTitle>
                  <DialogDescription>
                    Log a communication received from DG Shipping
                  </DialogDescription>
                </DialogHeader>
        ...
        <CardContent className="p-0">
          {filteredComms.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              No incoming communications found. Click "Record Incoming" to add one.
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredComms.map((comm) => (
                  <TableRow key={comm.id}>
                    ...
                    <TableCell className="text-sm">{comm.dg_office}</TableCell>
                    <TableCell>
          ...
          {selectedComm && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedComm.ref_no} • Received on {formatDate(selectedComm.created_at)}
                </DialogDescription>
              </DialogHeader>
